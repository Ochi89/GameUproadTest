...
{
	//	すでに存在している
	if (s_instance) { CommonDebug::Log(" [ Font.cpp ] : error : Instance has been created."); return; }

	//	nullptrのとき
	if (!s_instance) { Font::s_instance = new Font(); }

	//	作成失敗
	if (!s_instance) { CommonDebug::Log(" [ Font.cpp ] : error : Failed to create instance."); return; }
}
