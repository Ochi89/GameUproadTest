...
//	@brief	インスタンス生成
//-----------------------------------------------------------------------------
void Menu::CreateInstance()
{
	//	すでに存在している
	if (s_instance) { CommonDebug::Log(" [ Menu.cpp ] : error : Instance has been created."); return; }

	//	nullptrのとき
	if (!s_instance) { Menu::s_instance = new Menu(); }

	//	作成失敗
	if (!s_instance) { CommonDebug::Log(" [ Menu.cpp ] : error : Failed to create instance."); return; }
}
