...
//	@brief	静的メンバ変数の実態の作成
//-----------------------------------------------------------------------------
Opening	*Opening::s_instance = nullptr;
...
//	@brief	インスタンス生成
//-----------------------------------------------------------------------------
void Opening::CreateInstance()
{
	//	すでに存在している
	if (s_instance) { CommonDebug::Log(" [ Opening.cpp ] : error : Instance has been created."); return; }

	//	nullptrのとき
	if (!s_instance) { Opening::s_instance = new Opening(); }

	//	作成失敗
	if (!s_instance) { CommonDebug::Log(" [ Opening.cpp ] : error : Failed to create instance."); return; }
}

//-----------------------------------------------------------------------------
//	@brief	インスタンス削除
//-----------------------------------------------------------------------------
void Opening::DeleteInstance()
{
