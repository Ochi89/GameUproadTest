...
{
	//	すでに存在している
	if (s_instance) { CommonDebug::Log(" [ Mouse.cpp ] : error : Instance has been created."); return; }

	//	nullptrのとき
	if (!s_instance) { Mouse::s_instance = new Mouse(); }

	//	作成失敗
	if (!s_instance) { CommonDebug::Log(" [ Mouse.cpp ] : error : Failed to create instance."); return; }
}
