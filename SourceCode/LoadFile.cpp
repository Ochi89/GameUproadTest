...
//	@brief	静的メンバ変数の実態の作成
//-----------------------------------------------------------------------------
LoadFile	*LoadFile::s_instance = nullptr;
...
//	@brief	インスタンス生成
//-----------------------------------------------------------------------------
void LoadFile::CreateInstance()
{
	//	すでに存在している
	if (s_instance) { CommonDebug::Log(" [ LoadFile.cpp ] : error : Instance has been created."); return; }

	//	nullptrのとき
	if (!s_instance) { LoadFile::s_instance = new LoadFile(); }

	//	作成失敗
	if (!s_instance) { CommonDebug::Log(" [ LoadFile.cpp ] : error : Failed to create instance."); return; }
}

//-----------------------------------------------------------------------------
//	@brief	インスタンス削除
//-----------------------------------------------------------------------------
void LoadFile::DeleteInstance()
{
